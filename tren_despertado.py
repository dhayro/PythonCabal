...
def imagenmostruo():
    ...
    if search == False:
        ...
        return percentage
def funcionmostruo():
    global stop_flag, contadormostruos1, bosfinal, dungeon, puerta, terminar, bosdetectado,etapa,atacar,final
    ...
    while not stop_flag and not keyboard.is_pressed('delete'):
        ...
        imagen_a_buscar_bosfinal = "tren/bosfinal.png"  # Reemplaza con la ruta de tu propia imagen
        ...
        if vidamostruo:
            ...
            keyboard.press_and_release("z")
            contadormostruos1 += 1
            print('variable dhayri')
        if contadormostruos1>=6 and dungeon==1 and final==0:
            etapa=3
            print('variable dhayro')
...
def click_raton_posicion (x,y):
    ...
    time.sleep(0.5)
def funcionfail():
    global stop_flag,muriendo,esperandocuenta,muro2,bosscuenta,wavecuenta,etapa,final, dungeon, nombre_proceso, tiempo_inicio, puerta, terminando, terminar, lanzabuff, puertita, atacar, muriendo, fail
    ...
    def reset_variables():
        ...
        dungeon = 2
        bosdetectado = terminando = lanzabuff = atacar = bosscuenta = wavecuenta  = puerta = puertita = esperandocuenta = muriendo = muriendo =etapa = final = 0
...
def funcioiniciar():
    ...
    while not stop_flag and not keyboard.is_pressed('delete'):
        ...
        time.sleep(0.5)
        if not iniciar and  salix and dungeon!=2 and not buscar_imagen_en_pantalla("otros/dungeo.png"):
            dungeon=1
...
def handle_tronco():
    entrada = buscar_imagen_en_pantalla("tren/entrada.png")
    if entrada:
        raton_posicion(entrada[0], entrada[1])
    confirmatronco = buscar_imagen_en_pantalla("tren/confirmatronco.png")
    if confirmatronco:
        pyautogui.scroll(-10000)
        click_imagen(confirmatronco, offset_x=-20)
        time.sleep(1)
...
def handle_dungeon_selection():
    ...
    if selectdg:
        ...
        tren = imagentren()
        detected = buscar_imagen_en_pantalla("tren/detected.png")
        if not detected and tren:
            click_imagen(tren)
            time.sleep(1)
            raton_posicion(tren[0], tren[1]+30)
        if tren and detected:
            print('clic tren')
...
def imagenterminar():
    try:
        ...
        return False
def funcionterminar():
    global stop_flag, terminar,muro2,muriendo,final, dungeon, bosdetectado, vidamostruo, puerta,etapa, terminando, lanzabuff, nombre_proceso, tiempo_inicio, contardgok, atacar
    ...
    while not stop_flag and not keyboard.is_pressed('delete'):
        ...
        if dungeo and dungeon == 1:
            dungeon, bosdetectado, terminando, atacar, lanzabuff,muro2,muriendo,etapa,final = 2, 0, 0, 0, 0,False,0,0,0
            contardgok += 1
            tiempo_inicio = time.time()
            time.sleep(1.5)
...
def funcionetapa():
    ...
    while not stop_flag and not keyboard.is_pressed('delete'):
        ...
        time.sleep(1)
        if etapa>0 and vidamostruo==False  and dungeon==1 and not buscar_imagen_en_pantalla("otros/terminar.png") and bosfinal==False and terminar== False:
            if etapa==3:
                print('etapa 3')
                conteo=0
                ...
                while True:
                    ...
                    if vidamostruo!=False:
                            ...
                            atacar=1
                            break
                    if etapa>0 and vidamostruo==False   and dungeon==1 and bosfinal==False and terminar== False: #and contadormostruos1>=3
                        print('etapa kong')
                        # keyboard.press_and_release("z")
                        if vidamostruo!=False:
                            etapa=0
                        ...
                        if contadormostruos1>20:
                            ...
                            if conteo > 5:
                                ...
                                valore=170
                                valore1=20
                        imagen_a_buscar_pared8 = "tren/pared8.png"  # Reemplaza con la ruta de tu propia imagen
                        pared8 = buscar_imagen_en_pantalla(imagen_a_buscar_pared8)
                        if pared8!= False :
                            click_raton_posicion (pared8[0]-190, pared8[1]+70)
                        imagen_a_buscar_pared4 = "tren/pared4.png"  # Reemplaza con la ruta de tu propia imagen
                        pared4 = buscar_imagen_en_pantalla(imagen_a_buscar_pared4)
                        if pared4!= False :
                            click_raton_posicion (pared4[0]-240, pared4[1]-75)
                        if centro_ventana3:
                            print(f"El centro de la ventana3 de {nombre_proceso} es ({centro_ventana3[0]}, {centro_ventana3[1]}).")
                            raton_posicion (centro_ventana3[0]+valore1, centro_ventana3[1]-valore)
